...
- **handle** – handle events/actions (e.g. `handle_click` in script; wire to the framework’s event surface—`onclick` in Svelte, `onClick` in React).
...
**Stack:** SvelteKit (Vite), TypeScript, Panda CSS.

- **`src/routes/`** — file-based routes (`+layout.svelte`, `+page.svelte`, …).
- **`src/lib/components/`** — reusable UI; prefer **one folder per component** (e.g. `button/button.svelte`, `button/button.recipe.ts`). Icons: `components/icons/icon-*.svelte`.
- **`src/lib/features/`** — larger sections or page compositions (intro, skills, projects, …).
- **`src/lib/utils/`** — plain TypeScript helpers.
- **`src/images/`** — image assets.
- **`src/app.html`**, **`src/app.css`** — app shell; global CSS entry and Panda layers.
- **`styled-system/`** — Panda codegen output (**do not edit by hand**); drive it from `panda.config.ts` and the project’s codegen script.
- **`.cursor/AGENTS.md`** — this file (Cursor copy of guidelines; root `AGENTS.md` may differ).

**Path aliases** (from `svelte.config.js` → `kit.alias`):

- **`@components`** → `src/lib/components`
- **`@features`** → `src/lib/features`
- **`@utils`** → `src/lib/utils`
- **`@icons`** → `src/lib/icons` (alias exists for optional use; icon components are currently under **`@components/icons/...`**)
- **`styled-system`** → repo root `styled-system/` (Panda output; also included in `tsconfig` for types)

Root tooling config: `panda.config.ts`, `vite.config.ts`, `svelte.config.js`, `postcss.config.cjs`, `tsconfig.json`.
...
This project uses **[Panda CSS](https://panda-css.com/)** with generated output in **`styled-system/`** (see `panda.config.ts`).

- Use **`css()`** and **`cx()`** from **`styled-system/css`**: `css` for style objects, `cx` to merge recipe classes with extra `class` values.
- Use **layout patterns** from **`styled-system/patterns`** (e.g. `vstack`, `hstack`) for stacks and other presets instead of re-specifying the same flex/grid rules.
- Use **recipes** from **`styled-system/recipes`** for components with variants (e.g. `button({ variant: 'primary' })`). Define recipes with **`defineRecipe`** in colocated **`*.recipe.ts`** files and register them under `theme.extend.recipes` in **`panda.config.ts`**. Add recipes to **`staticCss.recipes`** when they must be emitted for static extraction.
- Reference **tokens** with the **`$` prefix** (project hook formats names as `$a.b.c`): semantic shortcuts like **`$primary`**, **`$background`**, **`$body`**, **`$foreground`**, **`$border`**, **`$muted`**, spacing/radius/typography scales (**`$2`**, **`$md`**, **`$display`**, …), and explicit paths like **`{$colors.primary}`** in values when you need a specific token path or alpha (e.g. **`{$colors.primary}/30`**).
- Prefer **semantic tokens** and shared scales over hard-coded colors or magic numbers so light/dark and theme tweaks stay centralized (`semanticTokens` and extended `tokens` in config).
- Use Panda **conditions** for states and accessibility (e.g. **`_hover`**, **`_motionReduce`**, focus utilities like **`focusVisibleRing`**) instead of one-off global CSS when the design system already models them.
- Keep **document-level rules** in **`globalCss`** in `panda.config.ts` (e.g. base `html`/`body`, selection, global focus variables) rather than scattering resets across components.
...
- Always add an empty line after the last statement in a block.
