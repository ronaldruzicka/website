<script lang="ts">
	const oxc_mask_id = globalThis.crypto.randomUUID();
	const oxc_filter_id_0 = globalThis.crypto.randomUUID();
	const oxc_filter_id_1 = globalThis.crypto.randomUUID();
	const oxc_filter_id_2 = globalThis.crypto.randomUUID();
	const oxc_filter_id_3 = globalThis.crypto.randomUUID();
	const oxc_filter_id_4 = globalThis.crypto.randomUUID();
	const oxc_filter_id_5 = globalThis.crypto.randomUUID();
	const oxc_filter_id_6 = globalThis.crypto.randomUUID();
	const oxc_filter_id_7 = globalThis.crypto.randomUUID();
	const oxc_filter_id_8 = globalThis.crypto.randomUUID();
	const oxc_filter_id_9 = globalThis.crypto.randomUUID();
	const oxc_filter_id_10 = globalThis.crypto.randomUUID();
	const oxc_filter_id_11 = globalThis.crypto.randomUUID();
	const oxc_filter_id_12 = globalThis.crypto.randomUUID();
	const oxc_filter_id_13 = globalThis.crypto.randomUUID();
</script>

<svg role="img" aria-label="Oxc icon" fill="none" viewBox="0 0 48 46"
	><path
		fill="#32f3ef"
		d="M30.316 6.789a2.26 2.26 0 0 0 2.262 2.262h8.582c1.009 0 1.513 1.219.8 1.93L30.978 21.963a2.26 2.26 0 0 0-.664 1.6v3.755c0 1.562 1.55 2.645 2.92 1.9a20.4 20.4 0 0 0 3.868-2.727c.465-.416 1.18-.42 1.622.023l7.999 7.999a1.1 1.1 0 0 1-.013 1.585 33.82 33.82 0 0 1-23.184 9.153A33.82 33.82 0 0 1 .343 36.098a1.1 1.1 0 0 1-.013-1.585l7.999-7.999c.441-.441 1.157-.44 1.622-.023a20.4 20.4 0 0 0 3.869 2.727c1.372.745 2.919-.338 2.919-1.9v-3.755c0-.6-.24-1.176-.664-1.6L5.093 10.98c-.713-.713-.209-1.93.8-1.93h8.582a2.26 2.26 0 0 0 2.262-2.262V1.132a1.13 1.13 0 0 1 1.13-1.131H29.18a1.13 1.13 0 0 1 1.131 1.13V6.79z"
	/><mask
		id={oxc_mask_id}
		width="48"
		height="46"
		x="0"
		y="0"
		maskUnits="userSpaceOnUse"
		style="mask-type:alpha"
		><path
			fill="#d9d9d9"
			d="M30.316 6.788a2.26 2.26 0 0 0 2.262 2.262h8.582c1.009 0 1.513 1.219.8 1.93L30.978 21.962a2.26 2.26 0 0 0-.664 1.6v3.755c0 1.562 1.55 2.645 2.92 1.9a20.4 20.4 0 0 0 3.868-2.727c.465-.416 1.18-.42 1.622.024l7.999 7.998a1.1 1.1 0 0 1-.013 1.585 33.82 33.82 0 0 1-23.184 9.153A33.82 33.82 0 0 1 .343 36.097a1.1 1.1 0 0 1-.013-1.585l7.999-7.998c.441-.442 1.157-.44 1.622-.024a20.4 20.4 0 0 0 3.869 2.727c1.372.745 2.919-.338 2.919-1.9v-3.755c0-.6-.24-1.176-.664-1.6L5.093 10.98c-.713-.713-.209-1.93.8-1.93h8.582a2.26 2.26 0 0 0 2.262-2.262V1.13a1.13 1.13 0 0 1 1.13-1.131H29.18a1.13 1.13 0 0 1 1.131 1.13v5.658z"
		/></mask
	><g mask={`url(#${oxc_mask_id})`}
		><g filter={`url(#${oxc_filter_id_0})`}
			><ellipse
				cx="4.618"
				cy="21.501"
				fill="#aefffb"
				rx="4.618"
				ry="21.501"
				transform="matrix(0 -1 -1 0 44.898 50.594)"
			/></g
		><g filter={`url(#${oxc_filter_id_1})`}
			><ellipse
				cx="4.618"
				cy="21.501"
				fill="#0f8"
				rx="4.618"
				ry="21.501"
				transform="rotate(68.34 1.902 66.786)scale(-1 1)"
			/></g
		><g filter={`url(#${oxc_filter_id_2})`}
			><ellipse
				cx="5.201"
				cy="44.012"
				fill="#195eff"
				rx="4.618"
				ry="21.501"
				transform="rotate(-68.34 5.201 44.012)"
			/></g
		><g filter={`url(#${oxc_filter_id_3})`}
			><ellipse
				cx="31.927"
				cy="28.892"
				fill="#aefffb"
				rx="3.715"
				ry="6.268"
				transform="rotate(-135.197 31.927 28.892)"
			/></g
		><g filter={`url(#${oxc_filter_id_4})`}
			><ellipse
				cx="33.843"
				cy="25.445"
				fill="#195eff"
				rx="3.715"
				ry="6.268"
				transform="rotate(-135.197 33.843 25.445)"
			/></g
		><g filter={`url(#${oxc_filter_id_5})`}
			><ellipse
				cx="33.459"
				cy="6.297"
				fill="#aefffb"
				rx="3.715"
				ry="6.268"
				transform="rotate(-135.197 33.46 6.297)"
			/></g
		><g filter={`url(#${oxc_filter_id_6})`}
			><ellipse
				cx="34.608"
				cy="4.765"
				fill="#195eff"
				rx="3.715"
				ry="6.268"
				transform="rotate(-135.197 34.608 4.765)"
			/></g
		><g filter={`url(#${oxc_filter_id_7})`}
			><ellipse
				cx="3.715"
				cy="6.268"
				fill="#aefffb"
				rx="3.715"
				ry="6.268"
				transform="rotate(135.197 1.052 21.915)scale(-1 1)"
			/></g
		><g filter={`url(#${oxc_filter_id_8})`}
			><ellipse
				cx="3.715"
				cy="6.268"
				fill="#195eff"
				rx="3.715"
				ry="6.268"
				transform="rotate(135.197 .422 19.64)scale(-1 1)"
			/></g
		><g filter={`url(#${oxc_filter_id_9})`}
			><ellipse
				cx="3.715"
				cy="6.268"
				fill="#aefffb"
				rx="3.715"
				ry="6.268"
				transform="rotate(135.197 4.526 9.682)scale(-1 1)"
			/></g
		><g filter={`url(#${oxc_filter_id_10})`}
			><ellipse
				cx="3.715"
				cy="6.268"
				fill="#195eff"
				rx="3.715"
				ry="6.268"
				transform="rotate(135.197 4.808 8.455)scale(-1 1)"
			/></g
		><g filter={`url(#${oxc_filter_id_11})`}
			><ellipse
				cx="3.715"
				cy="11.949"
				fill="#195eff"
				rx="3.715"
				ry="11.949"
				transform="rotate(135.197 1.56 17.42)scale(-1 1)"
			/></g
		><g filter={`url(#${oxc_filter_id_12})`}
			><ellipse
				cx="38.995"
				cy="14.521"
				fill="#0f8"
				rx="3.715"
				ry="11.949"
				transform="rotate(-135.197 38.995 14.521)"
			/></g
		><g filter={`url(#${oxc_filter_id_13})`}
			><ellipse
				cx="4.618"
				cy="21.501"
				fill="#195eff"
				rx="4.618"
				ry="21.501"
				transform="matrix(0 -1 -1 0 44.898 52.509)"
			/></g
		></g
	><defs
		><filter
			id={oxc_filter_id_0}
			width="61.384"
			height="27.618"
			x="-7.294"
			y="32.167"
			color-interpolation-filters="sRGB"
			filterUnits="userSpaceOnUse"
			><feFlood flood-opacity="0" result="BackgroundImageFix" /><feBlend
				in="SourceGraphic"
				in2="BackgroundImageFix"
				result="shape"
			/><feGaussianBlur
				result="effect1_foregroundBlur_2002_17235"
				stdDeviation="4.596"
			/></filter
		><filter
			id={oxc_filter_id_1}
			width="58.495"
			height="36.43"
			x="12.335"
			y="25.797"
			color-interpolation-filters="sRGB"
			filterUnits="userSpaceOnUse"
			><feFlood flood-opacity="0" result="BackgroundImageFix" /><feBlend
				in="SourceGraphic"
				in2="BackgroundImageFix"
				result="shape"
			/><feGaussianBlur
				result="effect1_foregroundBlur_2002_17235"
				stdDeviation="4.596"
			/></filter
		><filter
			id={oxc_filter_id_2}
			width="58.495"
			height="36.43"
			x="-24.046"
			y="25.797"
			color-interpolation-filters="sRGB"
			filterUnits="userSpaceOnUse"
			><feFlood flood-opacity="0" result="BackgroundImageFix" /><feBlend
				in="SourceGraphic"
				in2="BackgroundImageFix"
				result="shape"
			/><feGaussianBlur
				result="effect1_foregroundBlur_2002_17235"
				stdDeviation="4.596"
			/></filter
		><filter
			id={oxc_filter_id_3}
			width="28.672"
			height="28.706"
			x="17.592"
			y="14.539"
			color-interpolation-filters="sRGB"
			filterUnits="userSpaceOnUse"
			><feFlood flood-opacity="0" result="BackgroundImageFix" /><feBlend
				in="SourceGraphic"
				in2="BackgroundImageFix"
				result="shape"
			/><feGaussianBlur
				result="effect1_foregroundBlur_2002_17235"
				stdDeviation="4.596"
			/></filter
		><filter
			id={oxc_filter_id_4}
			width="28.672"
			height="28.706"
			x="19.507"
			y="11.093"
			color-interpolation-filters="sRGB"
			filterUnits="userSpaceOnUse"
			><feFlood flood-opacity="0" result="BackgroundImageFix" /><feBlend
				in="SourceGraphic"
				in2="BackgroundImageFix"
				result="shape"
			/><feGaussianBlur
				result="effect1_foregroundBlur_2002_17235"
				stdDeviation="4.596"
			/></filter
		><filter
			id={oxc_filter_id_5}
			width="28.672"
			height="28.706"
			x="19.123"
			y="-8.055"
			color-interpolation-filters="sRGB"
			filterUnits="userSpaceOnUse"
			><feFlood flood-opacity="0" result="BackgroundImageFix" /><feBlend
				in="SourceGraphic"
				in2="BackgroundImageFix"
				result="shape"
			/><feGaussianBlur
				result="effect1_foregroundBlur_2002_17235"
				stdDeviation="4.596"
			/></filter
		><filter
			id={oxc_filter_id_6}
			width="28.672"
			height="28.706"
			x="20.272"
			y="-9.587"
			color-interpolation-filters="sRGB"
			filterUnits="userSpaceOnUse"
			><feFlood flood-opacity="0" result="BackgroundImageFix" /><feBlend
				in="SourceGraphic"
				in2="BackgroundImageFix"
				result="shape"
			/><feGaussianBlur
				result="effect1_foregroundBlur_2002_17235"
				stdDeviation="4.596"
			/></filter
		><filter
			id={oxc_filter_id_7}
			width="28.672"
			height="28.706"
			x="1.124"
			y="15.305"
			color-interpolation-filters="sRGB"
			filterUnits="userSpaceOnUse"
			><feFlood flood-opacity="0" result="BackgroundImageFix" /><feBlend
				in="SourceGraphic"
				in2="BackgroundImageFix"
				result="shape"
			/><feGaussianBlur
				result="effect1_foregroundBlur_2002_17235"
				stdDeviation="4.596"
			/></filter
		><filter
			id={oxc_filter_id_8}
			width="28.672"
			height="28.706"
			x="-1.557"
			y="11.859"
			color-interpolation-filters="sRGB"
			filterUnits="userSpaceOnUse"
			><feFlood flood-opacity="0" result="BackgroundImageFix" /><feBlend
				in="SourceGraphic"
				in2="BackgroundImageFix"
				result="shape"
			/><feGaussianBlur
				result="effect1_foregroundBlur_2002_17235"
				stdDeviation="4.596"
			/></filter
		><filter
			id={oxc_filter_id_9}
			width="28.672"
			height="28.706"
			x="-1.557"
			y="-8.055"
			color-interpolation-filters="sRGB"
			filterUnits="userSpaceOnUse"
			><feFlood flood-opacity="0" result="BackgroundImageFix" /><feBlend
				in="SourceGraphic"
				in2="BackgroundImageFix"
				result="shape"
			/><feGaussianBlur
				result="effect1_foregroundBlur_2002_17235"
				stdDeviation="4.596"
			/></filter
		><filter
			id={oxc_filter_id_10}
			width="28.672"
			height="28.706"
			x="-1.939"
			y="-10.353"
			color-interpolation-filters="sRGB"
			filterUnits="userSpaceOnUse"
			><feFlood flood-opacity="0" result="BackgroundImageFix" /><feBlend
				in="SourceGraphic"
				in2="BackgroundImageFix"
				result="shape"
			/><feGaussianBlur
				result="effect1_foregroundBlur_2002_17235"
				stdDeviation="4.596"
			/></filter
		><filter
			id={oxc_filter_id_11}
			width="36.034"
			height="36.133"
			x="-8.858"
			y="-.482"
			color-interpolation-filters="sRGB"
			filterUnits="userSpaceOnUse"
			><feFlood flood-opacity="0" result="BackgroundImageFix" /><feBlend
				in="SourceGraphic"
				in2="BackgroundImageFix"
				result="shape"
			/><feGaussianBlur
				result="effect1_foregroundBlur_2002_17235"
				stdDeviation="4.596"
			/></filter
		><filter
			id={oxc_filter_id_12}
			width="36.034"
			height="36.133"
			x="20.978"
			y="-3.545"
			color-interpolation-filters="sRGB"
			filterUnits="userSpaceOnUse"
			><feFlood flood-opacity="0" result="BackgroundImageFix" /><feBlend
				in="SourceGraphic"
				in2="BackgroundImageFix"
				result="shape"
			/><feGaussianBlur
				result="effect1_foregroundBlur_2002_17235"
				stdDeviation="4.596"
			/></filter
		><filter
			id={oxc_filter_id_13}
			width="61.384"
			height="27.618"
			x="-7.294"
			y="34.082"
			color-interpolation-filters="sRGB"
			filterUnits="userSpaceOnUse"
			><feFlood flood-opacity="0" result="BackgroundImageFix" /><feBlend
				in="SourceGraphic"
				in2="BackgroundImageFix"
				result="shape"
			/><feGaussianBlur
				result="effect1_foregroundBlur_2002_17235"
				stdDeviation="4.596"
			/></filter
		></defs
	></svg
>
