<script lang="ts">
	const tanstack_b = globalThis.crypto.randomUUID();
	const tanstack_d = globalThis.crypto.randomUUID();
	const tanstack_e = globalThis.crypto.randomUUID();
	const tanstack_f = globalThis.crypto.randomUUID();
	const tanstack_g = globalThis.crypto.randomUUID();
	const tanstack_h = globalThis.crypto.randomUUID();
	const tanstack_i = globalThis.crypto.randomUUID();
	const tanstack_j = globalThis.crypto.randomUUID();
	const tanstack_k = globalThis.crypto.randomUUID();
	const tanstack_l = globalThis.crypto.randomUUID();
	const tanstack_m = globalThis.crypto.randomUUID();
	const tanstack_n = globalThis.crypto.randomUUID();
	const tanstack_o = globalThis.crypto.randomUUID();
	const tanstack_p = globalThis.crypto.randomUUID();
	const tanstack_q = globalThis.crypto.randomUUID();
	const tanstack_r = globalThis.crypto.randomUUID();
	const tanstack_s = globalThis.crypto.randomUUID();
	const tanstack_t = globalThis.crypto.randomUUID();
	const tanstack_u = globalThis.crypto.randomUUID();
	const tanstack_a = globalThis.crypto.randomUUID();
	const tanstack_v = globalThis.crypto.randomUUID();
	const tanstack_c = globalThis.crypto.randomUUID();
</script>

<svg role="img" aria-label="TanStack icon" viewBox="0 0 633 633"
	><defs
		><linearGradient id={tanstack_b} x1="50%" x2="50%" y1="0%" y2="71.65%"
			><stop offset="0%" stop-color="#6BDAFF" /><stop
				offset="31.922%"
				stop-color="#F9FFB5"
			/><stop offset="70.627%" stop-color="#FFA770" /><stop
				offset="100%"
				stop-color="#FF7373"
			/></linearGradient
		><linearGradient
		...
		><linearGradient id={tanstack_u} x1="50.495%" x2="49.68%" y1="0%" y2="100%"
			><stop offset="0%" stop-color="#FFA400" /><stop
				...
				stop-color="#FF5E00"
			/></linearGradient
		><circle id={tanstack_a} cx="308.5" cy="308.5" r="308.5" /><circle
			id={tanstack_v}
			cx="307.5"
			cy="308.5"
			r="316.5"
	...
	><g fill="none" fill-rule="evenodd" transform="translate(9 8)"
		><mask id={tanstack_c} fill="#fff"
			><use xlink:href={`#${tanstack_a}`} /></mask
		><use xlink:href={`#${tanstack_a}`} fill={`url(#${tanstack_b})`} /><ellipse
			cx="81.5"
			cy="602.5"
			fill="#015064"
			stroke="#00CFE2"
